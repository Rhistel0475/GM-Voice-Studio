"""
Kani TTS provider implementation.
"""
...
def generate(
    ...
    try:
        ...
        if emb is not None:
            audio, _out_text = model(text, language_tag=normalized_language_tag, speaker_emb=emb, **kwargs)
        else:
        ...
        silence = np.zeros(int(KANI_SAMPLE_RATE * 1.0), dtype=np.float32)
        logging.warning("Returning 1.0s silence due to TTS failure; narration continues.")
