...
.pt files remain in local or S3; this layer only stores voice_id, name, consent_scope, created_at, owner_id, faction.
When DATABASE_URL is set, run voice migrations first: alembic -c alembic_voice.ini upgrade head
"""
import os
import sqlite3
...
def _sqlite_path() -> str:
    ...
    if u == "sqlite://" or u.startswith("sqlite:"):
        return str(Path(__file__).resolve().parent.parent.parent / "voice_metadata.db")
    return u
